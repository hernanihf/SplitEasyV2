...
type PushService interface {
	SetPushEnabled(ctx context.Context, userID uint, enabled bool) error
	...
	// actorID, skipping members who have disabled push. The notification's
	// title is always the group's name; its body is produced by bodyFor,
	// given the actor's display name — both resolved from the single group
	// fetch this method already does (no extra query per caller). Per-
	// subscription send failures are logged and swallowed — one dead
	// subscription must never stop the others from being notified. Meant to
	// be called from a goroutine the caller spawns after writing the
	// triggering request's response, using a context that outlives the
	// request (not r.Context(), which is canceled as soon as the handler
	// returns).
	NotifyGroupMembers(ctx context.Context, groupID, actorID uint, bodyFor func(actorName string) string, data map[string]string) error
}
...
func (s *pushService) SetPushEnabled(ctx context.Context, userID uint, enabled bool) error {
	return s.userRepo.UpdatePushEnabled(ctx, userID, enabled)
}
...
func (s *pushService) NotifyGroupMembers(ctx context.Context, groupID, actorID uint, bodyFor func(actorName string) string, data map[string]string) error {
