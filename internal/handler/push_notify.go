...
func notifyGroupMembersAsync(pushService service.PushService, groupID, actorID uint, category service.PushCategory, bodyFor func(actorName string) string) {
	if pushService == nil {
		return
	}
	go func() {
		data := map[string]string{"url": fmt.Sprintf("/groups/%d", groupID)}
		if err := pushService.NotifyGroupMembers(context.Background(), groupID, actorID, category, bodyFor, data); err != nil {
